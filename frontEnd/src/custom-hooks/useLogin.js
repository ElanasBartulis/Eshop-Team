...
export default function useLogin() {
  ...
  useEffect(() => {
    async function checkSession() {
      const promise = await fetch("http://srv701413.hstgr.cloud/server/api/users/session");
  ...
  async function onLogin(e) {
    ...
    try {
      const promise = await fetch("http://srv701413.hstgr.cloud/server/api/users/login", {
        method: "POST",
        headers: {
