...
export default function UserList() {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch("http://srv701413.hstgr.cloud/server/api/users/users");
      if (!response.ok) {
        setErrorHandler({
          isSnackbarOpen: true,
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await fetch(`http://srv701413.hstgr.cloud/server/api/users/users/${deleteConfirmation.userId}`, {
        method: "DELETE",
      });
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await fetch(`http://srv701413.hstgr.cloud/server/api/users/users/${editUser.id}`, {
        method: "PUT",
        headers: {
